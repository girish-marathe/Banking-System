...
public class AddEmployee extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String tname = "", lname = "", address = "", gender = "", email = "", mobile = "", adhar_id = "", pan_id = "", dob = "";
        String imgPath = "", status = "deactive";
        String branch = "";
        String password = "", jdate = "";
        String uname = "", desiganition = "";
        ...
        try {
            ...
            while (i.hasNext()) {
                ...
                if (!fi.isFormField()) {
                    ...
                    if (fieldname.trim().equals("adhar_id")) {
                        adhar_id = value.trim();
                    }
                    if (fieldname.trim().equals("pan_id")) {
                        pan_id = value.trim();
                    }
                    ...
                    if (fieldname.trim().equals("jdate")) {
                        jdate = value.trim();
                    }
                    if (fieldname.trim().equals("uname")) {
                        uname = value.trim();
                    }
                    if (fieldname.trim().equals("pass")) {
                        password = value.trim();
                    }
                    if (fieldname.trim().equals("designation")) {
                        desiganition = value.trim();
                    }
                ...
                String sql = "Select * from tbl_manager where uname='" + uname + "'and email='" + email + "'";
                rs = db.Select(sql);
                if (rs.next()) {
                    httpSession.setAttribute("uname", uname);
                    out.println("<script type=\"text/javascript\">");
                    out.println("alert('Employee has already exists!');");
                    out.println("location='managerHome.jsp';");
                    out.println("</script>");
                    return;
                } else {

                    String sql1 = "Insert into tbl_manager(branch,fname,lname,uname,pass,address,email,gender,mobile,adhar,pan,bdate,jdate,imgPath,status,desiganation)values('" + branch.trim() + "','" + tname.trim() + "','" + lname.trim() + "','" + uname.trim() + "','" + password.trim() + "','" + address.trim() + "','" + email.trim() + "','" + gender.trim() + "','" + mobile.trim() + "','" + adhar_id.trim() + "','" + pan_id.trim() + "','" + dob.trim() + "','" + jdate.trim() + "','" + imgPath.trim() + "','" + status.trim() + "','" + desiganition + "')";
                    int result = db.Insert(sql1);
                    if (result > 0) {
                        httpSession.setAttribute("uname", uname);
                        out.println("<script type=\"text/javascript\">");
                        out.println("alert('Adding new branch Employee successfully!');");
                        out.println("location='managerHome.jsp';");
                        out.println("</script>");
                        return;
                    } else {
                        httpSession.setAttribute("uname", uname);
                        out.println("<script type=\"text/javascript\">");
                        out.println("alert('Branch Employee has not added!');");
